...
prod-build: dev-build
	...
	docker build -t troeger/ore-front:0.8.1 -f frontend/docker/Dockerfile.production .
	docker build -t troeger/ore-back:0.8.1 -f backends/docker/Dockerfile.production .

prod-push:
	docker login
	docker push troeger/ore-front:0.8.1 
	docker push troeger/ore-back:0.8.1
