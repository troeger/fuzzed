...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # removed old 'probabilitySelected' field and stores the information in the 'probability' field
        for oldProperty in orm.Property.objects.filter(key='probabilitySelected'):
            node = oldProperty.node
            index = 2 if oldProperty.key == 'Fuzzy' else 0  # 'Exact'
            prob_property = node.properties.get(key='probability')
            prob_property.value = [index, prob_property.value]
            prob_property.save()
            oldProperty.delete()
    ...
    models = {
        ...
        'FuzzEd.job': {
            ...
            'secret': ('django.db.models.fields.CharField', [], {'default': "'e43ec809-0f88-4a2f-8f2a-85786afc576e'", 'max_length': '64'})
        },
        'FuzzEd.node': {
