define(['config', 'jquery'], function(Config) {
     ...
     *
     * Parameters:
     *   {str} type        - The message kind identifier string. Styles the message accordingly. Should equal to one of
     *                       Bootstrap's predefined CSS classes for styling alert messages (see: Components > Alerts)
     *   {str} message     - The title of the alert message. Should be a brief one liner.
     *   {str} description - Details of the given message.
     *   {int} timeout     - The time in milliseconds after which the alert shall fade out. If no timeout is given the
     *                       alert will remain active until explicitly closed by the user. A hover over the alert box
     *                       resets the timeout. This mean there is no guaranteed fade after the given time.
     *
     * Returns:
     *   undefined
     */
