...
AVAILABLE_OPTIONS = dict(
	package=["apt", "yum", "zypper", "pacman", "emerge", "pkgin", "homebrew"],
	python_package=["pip", "easy_install"]
)

PACKAGE_EXECUTABLE = {	'apt':'apt-get', 
			'yum':'yum', 
			'zypper':'zypper', 
			'pacman':'pacman', 
			'emerge':'emerge', 
...
DEFAULT_OPTIONS = dict(package=None, python_package=None)
...
def detect_system_defaults():
	""" Sets DEFAULT_OPTIONS to reasonable values based on the given system. We rely
	on the run() wrapper to do that on the correct host."""
	for package in AVAILABLE_OPTIONS['package']:
		package_exec = PACKAGE_EXECUTABLE[package]
		if package_exec in run_local("which "+package_exec):
			DEFAULT_OPTIONS['package'] = package
			break
	else:
		print("Warning: None of the supported package managers was found: %s"%str(AVAILABLE_OPTIONS["package"]))

	for python_package in AVAILABLE_OPTIONS['python_package']:
		python_package_exec = PACKAGE_EXECUTABLE[python_package]
		if python_package_exec in run_local("which "+python_package_exec):
			DEFAULT_OPTIONS['python_package'] = python_package
			break
	else:
		print("Warning: None of the supported python package managers was found: %s"%str(AVAILABLE_OPTIONS["python_package"]))
...
def repository_ensure_apt(repository):
	...
	return sudo("add-apt-repository --yes " + repository)
...
# -----------------------------------------------------------------------------
# Homebrew (OSX)
# added by troeger - 20130818 - v0.1
# -----------------------------------------------------------------------------

def homebrew_result(result):
	if result.return_code != 0:
		# Catch the case that Homebrew is simply not installed
		if "brew" not in run("which brew"):
			raise Exception("Homebrew is not installed. Choose another package provider, or check http://brew.sh for installation instructions.")
	return result

def repository_ensure_homebrew(repository):
	# This doesn't make sense, since homebrew supports formula URLs
	raise Exception("Not implemented for Homebrew.")

def package_upgrade_homebrew():
	return homebrew_result( run("brew upgrade") )

def package_update_homebrew(package=None):
	if package == None:
		return homebrew_result( run("brew update") )
	else:
		if type(package) in (list, tuple):
			package = " ".join(package)
		return homebrew_result( run("brew upgrade "+package) )

def package_install_homebrew(package, update=False):
	if update:
		package_update_homebrew()
	if type(package) in (list, tuple):
		package = " ".join(package)
	return homebrew_result( run("brew install "+package) )

def package_ensure_homebrew(package, update=False):
	#TODO: This function cannot return a single run() result,
	#      since update / installation for multiple packages cannot
	#      be performed by one command call in Homebrew.
	#      The no-brew-installed case is catched anyway.
	if type(package) not in (list, tuple):
		package = [package]
	for p in package:
		output = run("brew list -l | grep %s ; true" % p)
		for line in output:
			if line.endswith(p):
				# Got it, update it in case
				if update:
					package_update_homebrew(p)
		# Not found
		package_install_homebrew(p)

def package_clean_homebrew(package=None):
	if package:
		if type(package) in (list, tuple):
			package = " ".join(package)
		homebrew_result( run("brew cleanup -s "+package) )
	else:
		homebrew_result( run("brew cleanup -s") )

def package_remove_homebrew(package, autoclean=False):
	if autoclean:
		package_clean_homebrew(package)
	homebrew_result( run("brew remove "+package) )
...
def python_package_upgrade_pip(package):
	...
	'''
	return run('pip install --upgrade %s' %(package))
...
def python_package_install_easy_install(package):
	...
	The "package" argument, defines the name of the package that will be installed.
	'''
	return sudo('easy_install %s' %package)
...
def python_package_ensure_easy_install(package):
	...
	# So I just call the install functions
	return python_package_install_easy_install(package)
...
def user_check(name=None, uid=None, need_passwd=True):
	...
	if IS_DARWIN:
		...
		if name != None:
			...
			pwd_data = pwd.getpwuid(uid)
		results = dict(name=pwd_data[0], uid=pwd_data[2], gid=pwd_data[3], fullname=pwd_data[4], home=pwd_data[5], shell=pwd_data[6])
		if need_passwd:
			#TODO: Theoretically, this just needs a call to the spwd module in Python, but this is not available on my Mac
			#TODO: It also seems like dscl is not providing this functionality intentionally
			raise Exception("Password hash fetching is not supported on Mac OS X. Sorry.")
	else:
		if   name != None:
...
def _init():
	...
	if "sudo" not in run_local("which sudo"):
		print("WARNING: sudo is not installed.")
