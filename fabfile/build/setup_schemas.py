COMMON_XSD = '''<?xml version="1.0" encoding="UTF-8" standalone="no"?>
...
def createFaultTreeSchema(file_name):
    create(
        file_name, 
        COMMON_XSD.format('net.faulttree', 'FaultTree', 'ft:ChildNode', 'ft:ChildNode'), 
        FAULTTREE_XSD
    )

def createFuzzTreeSchema(file_name):
    create(
        file_name, 
        COMMON_XSD.format(
            'net.fuzztree', 'FuzzTree', 'ft:VariationPoint', 
            'ft:InclusionVariationPoint'
        ), 
        FUZZTREE_XSD
    )

def create(file_name, head, custom):
    xsd = [head, custom, '</xsd:schema>']
    print 'Writing new XSD file to ', file_name

    with open(file_name, 'w') as handle:
        handle.write(''.join(xsd))
