...
from .common import fixt_simple, OreLiveServerTestCase, OreTestCase

import logging
logging.basicConfig(level=logging.DEBUG)


class ExternalAPITestCase(OreLiveServerTestCase):
...
class GraphMLFilesTestCase(OreTestCase):
    ...
    def testImportFiles(self):
        files = [
            f for f in os.listdir('ore/fixtures') if f.endswith(".graphml")]
        print files
        for f in files:
            print("Testing " + f)
            text = open('ore/fixtures/' + f).read()
            # Now import the same GraphML
            response = self.postWithAPIKey(
